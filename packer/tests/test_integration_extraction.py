...
def test_single_volume_extraction(tmp_path: Path, make_cbz, run_packer):
    ...
    for i in range(1, 4):
        ...
        assert moved_archive.exists(), f"archive not found in volume dir: {moved_archive}"
        ...
        assert (chapter_dir / "001.jpg").exists(), (
            f"extracted image missing in {chapter_dir}"
        )
...
@pytest.mark.integration
def test_chapter_extras_extracted_alongside_main(
    tmp_path: Path, make_cbz, run_packer
):
...
@pytest.mark.integration
def test_batch_processing_creates_two_volumes(
    tmp_path: Path, make_cbz, run_packer
):
    ...
    for i in range(4, 7):
        assert not (vol1_dir / f"Chapter {i:03d}").exists(), (
            f"chapter {i} leaked into vol01"
        )
    for i in range(1, 4):
        assert not (vol2_dir / f"Chapter {i:03d}").exists(), (
            f"chapter {i} leaked into vol02"
        )
