...
def test_process_one_normal(tmp_path: Path):
    ...
    cfg = Config(path=str(src), dest=str(dest), serie='S', volume=1, chapter_range=[1], nb_worker=1, dry_run=False, verbose=False, force=False)
...
def test_process_one_dry_run(tmp_path: Path):
    ...
    cfg = Config(path=str(src), dest=str(dest), serie='S', volume=1, chapter_range=[2], nb_worker=1, dry_run=True, verbose=True, force=False)
...
def test_process_one_force_overwrite(tmp_path: Path):
    ...
    cfg = Config(path=str(src), dest=str(dest), serie='S', volume=1, chapter_range=[3], nb_worker=1, dry_run=False, verbose=False, force=True)
...
def test_process_one_skip_if_chapter_exists(tmp_path: Path):
    ...
    cfg = Config(path=str(src), dest=str(dest), serie='S', volume=1, chapter_range=[4], nb_worker=1, dry_run=False, verbose=False, force=False)
    ...
    assert (dest / 'S v01' / 'Chapter 4.cbz'.lstrip()).exists() or (dest / 'S v01' / 'Chapter 4.cbz'.replace('Chapter ', 'Chapter ').lstrip()).exists()
...
def test_process_volume_removes_moved_files(tmp_path: Path):
    ...
    cfg = Config(path=str(src), dest=str(dest), serie='S', volume=1, chapter_range=[1, 2, 3], nb_worker=2, dry_run=False, verbose=False, force=False)
...
def test_process_volume_missing_chapter_returns_3(tmp_path: Path):
    ...
    cfg = Config(path=str(src), dest=str(dest), serie='S', volume=1, chapter_range=[1, 2], nb_worker=1, dry_run=False, verbose=False, force=False)
